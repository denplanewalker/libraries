...
local Library do 
    ...
    function remove_player_from_lists(player)
        for i, v in Library.Lists.Neutral do
            if v == player then
                table.remove(Library.Lists.Neutral, i)
                break
            end
        end
        for i, v in Library.Lists.Friendly do
            if v == player then
                table.remove(Library.Lists.Friendly, i)
                break
            end
        end
        for i, v in Library.Lists.Priority do
            if v == player then
                table.remove(Library.Lists.Priority, i)
                break
            end
        end
    end
    ...
    local Instances = { } do
        ...
        Instances.Tooltip = function(self, Text)
            ...
            local Tooltip = Instances:Create("Frame", {
                ...
                BorderSizePixel = 0,
                Size = UDim2New(0, 0, 0, 28), -- UDim2New(0, 0, 0, 22)
    ...
    local STIXZZ_CFG_OBF, STIXZZ_CFG_DEOBF do 
        local _SHIFT, _REFT, _PTF, _OLM = 523, 112, 32, 775
        local dec_key = tostring(_SHIFT+_REFT/2/_PTF/_OLM):sub(0, 5)
        ...
        local repeat_key = function(key, length)
            if #key >= length then return key:sub(1, length) end
            local result = ''
            for i = 1, math.floor(length / #key) do
                result = result .. key
            end
            if length % #key > 0 then
                result = result .. key:sub(1, length % #key)
            end
            return result
        end
        local xor = function(message, key)
            ...
            local result = ''
            for i = 1, #message do result = result .. string.char(bit32.bxor(message:sub(i, i):byte(), rkey:sub(i, i):byte())) end
            return result
        end 

        STIXZZ_CFG_OBF = function(file)
            return hexencode(xor(file, dec_key))
        end;

        STIXZZ_CFG_DEOBF = function(content)
            local message = hexdecode(content) 
            return xor(message, dec_key) 
        end;
    ...
    Library.GetConfig = function(self)
        ...
        return STIXZZ_CFG_OBF(HttpService:JSONEncode(Config))
    end

    Library.LoadConfig = function(self, Config)
        local Decoded = HttpService:JSONDecode(STIXZZ_CFG_DEOBF(Config))
    ...
    Library.DeleteConfig = function(self, Config)
        if isfile(Library.Folders.Configs .. "/" .. Config) then 
            ...
            Library:Notification("Deleted config " .. Config, 5, FromRGB(0, 255, 0))
        end
    end
    ...
    Library.SaveConfig = function(self, Config)
        if isfile(Library.Folders.Configs .. "/" .. Config) then
            ...
            Library:Notification("Saved config " .. Config, 5, FromRGB(0, 255, 0))
        end
    end
    ...
    Library.KeybindList = function(self)
        ...
        local Items = { } do 
            Items["KeybindListOutline"] = Instances:Create("Frame", {
                ...
                Name = "\0",
                Position = UDim2New(0, 20, 0.5, 150),
        ...
        Components.Window = function(Data)
            local Items = { } do 
                ...
                if Data.Resizable then 
                    Items["Outline"]:MakeResizeable(Vector2New(Data.ResizableX, Data.ResizableY), Vector2New(9999, 9999))
                end
    ...
    Library.Window = function(self, Data)
        ...
        local Window = {
            ...
            Size = Data.Size or Data.size or UDim2New(0, 593, 0, 730),
        ...
        local Items = Components.Window({
            Position = UDim2New(0, Camera.ViewportSize.X / 2, 0, Camera.ViewportSize.Y / 2),
            Size = Window.Size,
            Parent = self.Holder,
            Draggable = true,
            Resizable = true,
            ResizableX = Window.Size.X.Offset,
            ResizableY = Window.Size.Y.Offset
        }) do 
            Items["Title"] = Instances:Create("Frame", {
                Parent = Items["Inline"].Instance,
                Name = "\0",
    ...
    Library.Pages.PlayerList = function(self, Data)
                ...
                Callback = function(Value)
                    if Playerlist.Player then
                        ...
                        if Value == "Neutral" then
                            ...
                            remove_player_from_lists(Playerlist.Player)
                            table.insert(Library.Lists.Neutral, Playerlist.Player)
                        elseif Value == "Priority" then
                            ...
                            remove_player_from_lists(Playerlist.Player)
                            table.insert(Library.Lists.Priority, Playerlist.Player)
                        elseif Value == "Friendly" then
                            ...
                            remove_player_from_lists(Playerlist.Player)
                            table.insert(Library.Lists.Friendly, Playerlist.Player)
                        else
                            Playerlist.Players[Playerlist.Player.Name].PlayerStatus:Tween(nil, {
                                TextColor3 = FromRGB(180, 180, 180)
                            })
        ...
        function Playerlist:Add(Player)
            ...
            local PlayerTeam = Instances:Create("TextLabel", {
                ...
                BorderColor3 = FromRGB(0, 0, 0),
                Text = tostring(Player:GetAttribute("Team")) or "None",
                Name = "\0",
            ...
            Library:Connect(Player:GetAttributeChangedSignal("Team"), function()
                PlayerTeam.Instance.Text = tostring(Player:GetAttribute("Team")) or "None"
            end)
